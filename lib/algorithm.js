...
const ScheduleAlgorithm = (function () {
  ...
  const SPECIAL_TYPES = ['产假', '事假', '休', '二线', '培训', '开会', '医疗保障', '脱产学习'];
    ...
    for (let docId in (dutyAssigned || {})) {
      ...
      for (let d = 0; d < 7; d++) {
        ...
        if ((hasGaoxinAM || hasZitongAM) && dutySlots.pm === '值班' && !hasExistingFlag(doc.id, d, 'am')) {
          ...
          newFlags.push({ doctorId: doc.id, dayIdx: d, slot: 'am', reason: `${doc.name} — 上午${clinicName}+下午值班，请手动处理` });
        }
        if ((hasGaoxinAM || hasZitongAM) && dutySlots.pm === '中班' && !hasExistingFlag(doc.id, d, 'pm')) {
          const clinicName = hasGaoxinAM ? '高新门诊' : '梓潼门诊';
          newFlags.push({ doctorId: doc.id, dayIdx: d, slot: 'pm', reason: `${doc.name} — 上午${clinicName}+下午中班，请手动处理` });
        }
        if (hasGeneralAM && dutySlots.pm === '值班' && !hasExistingFlag(doc.id, d, 'am'))
          newFlags.push({ doctorId: doc.id, dayIdx: d, slot: 'am', reason: `${doc.name} — 上午总院门诊+下午值班，请手动处理` });
        if (hasGeneralPM && dutySlots.pm === '中班' && !hasExistingFlag(doc.id, d, 'pm'))
          newFlags.push({ doctorId: doc.id, dayIdx: d, slot: 'pm', reason: `${doc.name} — 下午总院门诊+中班冲突，请手动处理` });
      }
    }
    return newFlags;
  }
