...
const ScheduleAPI = (function () {
  ...
  function buildDoctorsFromAPI(employees) {
    ...
    for (var i = 0; i < employees.length; i++) {
      ...
      var scheduleCategory = getField(emp, ['shiftCategories', 'scheduleCategory', 'scheduleType', '排班类别']);
      var employeeType = getField(emp, ['employeeType', 'empType', 'personType', '人员类别']);
      var employeeNumber = getField(emp, ['cardNo', 'empCode', 'workCode', 'jobNumber', '工号']);
      var employeeName = getField(emp, ['name', 'empName', 'employeeName', 'realName', 'displayName', '姓名', 'cardNo']);

      // 排班类别：数字 1=医疗, 2=规培；字符串 '医疗'/'规培'
      var isMedical = (scheduleCategory === 1 || scheduleCategory === '1' || scheduleCategory === '医疗');
      var isTraining = (scheduleCategory === 2 || scheduleCategory === '2' || scheduleCategory === '规培');
      if (!isMedical && !isTraining) continue;
      ...
      if (doc.id && doc.name) result.push(doc);
    }
    return result;
  }
