...
const ScheduleAPI = (function () {
  ...
  function _injectBridge() {
    ...
    try {
      ...
      console.log('[ScheduleAPI] 准备注入 bridge.js，URL:', bridgeUrl);
      ...
      script.onload = function () {
        console.log('[ScheduleAPI] ✅ bridge.js 脚本加载成功（等待就绪消息...）');
      };
      script.onerror = function (err) {
        console.error('[ScheduleAPI] ❌ bridge.js 脚本加载失败！', err);
      ...
      console.log('[ScheduleAPI] bridge.js <script> 标签已添加到 DOM');
  ...
  window.addEventListener('message', function (event) {
    ...
    if (event.data.type === '__SCHEDULE_BRIDGE_READY__') {
      console.log('[ScheduleAPI] ✅ 收到 bridge 就绪消息');
      _bridgeInjected = true;
    ...
    // API 响应消息
    if (event.data.type !== '__SCHEDULE_API_RES__') return;

    var id = event.data.id;
    var resolver = _pendingRequests.get(id);
    if (!resolver) {
      console.warn('[ScheduleAPI] 收到未知请求ID的响应:', id, '（可能已超时）');
      return;
    }
    _pendingRequests.delete(id);

    if (event.data.error) {
      console.error('[ScheduleAPI] ❌ 请求 #' + id + ' bridge 返回错误:', event.data.error);
      resolver.reject(new Error(event.data.error));
    } else {
      // 解析响应数据
      var respData = event.data.data;
      if (respData && typeof respData === 'object') {
        console.log('[ScheduleAPI] ✅ 请求 #' + id + ' 成功, error=' + respData.error + ', 数据类型=' + (Array.isArray(respData.data) ? 'Array(' + respData.data.length + ')' : typeof respData.data) + ', 完整keys=' + Object.keys(respData).join(','));
      } else {
        console.log('[ScheduleAPI] ✅ 请求 #' + id + ' 成功, 原始响应类型:', typeof respData);
      }
  ...
  async function request(url, options) {
    ...
    // 等待 bridge 就绪
    console.log('[ScheduleAPI] ⏳ 等待 bridge 就绪...');
    await _injectBridge();
    console.log('[ScheduleAPI] ✅ bridge 已就绪，开始发送请求');

    // 提取请求路径用于日志
    var urlPath = url.replace(BASE, '');
    console.log('[ScheduleAPI] 🚀 发起请求 #' + (_requestId + 1) + ':', options.method || 'GET', urlPath,
      options.body ? (' body长度=' + options.body.length) : '');
    ...
    return new Promise(function (resolve, reject) {
      ...
      console.error('[ScheduleAPI] ❌ 请求异常捕获:', urlPath, '错误:', err.message, '堆栈:', err.stack);
  ...
  function buildDoctorsFromAPI(employees) {
    if (!employees || !Array.isArray(employees)) {
      console.warn('[ScheduleAPI] buildDoctorsFromAPI: 输入无效, employees=', typeof employees);
      return [];
    }

    console.log('[ScheduleAPI] buildDoctorsFromAPI: 输入员工数=' + employees.length);
    ...
    console.log('[ScheduleAPI] buildDoctorsFromAPI: 按排班类别过滤后=' + filtered.length + ' (医疗/规培)');
    ...
    return filtered
      .map(emp => {
        ...
        if (!doctor.id || !doctor.name) {
          console.warn('[ScheduleAPI] buildDoctorsFromAPI: 跳过缺少id或name的记录', JSON.stringify(emp).substring(0, 200));
        }
        return doctor;
      })
