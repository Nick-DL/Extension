...
const ScheduleAPI = (function () {
   ...
   * @param {Array} batchData - 由 buildBatchData 产出的数组
   * @returns {Promise<{error, data}>}
   */
  async function appendUsualClass(batchData) {
    return postJson('/schedule/employee', { ids: null, schedules: batchData });
  }
  ...
  function buildDoctorsFromAPI(employees) {
    ...
    for (var i = 0; i < employees.length; i++) {
      ...
      // 排班类别判定：
      var isMedical = (emp.shiftCategories === 1 || emp.shiftCategories === '1');
      var isTraining = (emp.category === 4 || emp.category === '4' || emp.shiftCategories === '6' || emp.shiftCategories === 6);
    ...
    console.log('[ScheduleAPI] 加载员工:', employees.length, '→ 过滤后', result.length, '人');
    return result;
  }
  ...
  function buildBatchData(scheduleState, classMap, locationId, weekMonday, slotFilter) {
    ...
    for (let d = 0; d < 7; d++) {
      ...
      dates.push(fmtLocal(dt));
    }

    const slots = slotFilter ? [slotFilter] : ['am', 'pm'];
    for (let doc of doctors) {
      for (let d = 0; d < 7; d++) {
        for (let s of slots) {
          ...
          if (classId) {
            // deptId 从医生原始数据中取
            const deptId = (doc._raw && doc._raw.deptId) ? doc._raw.deptId : null;
            batchData.push({
              empId: String(doc.id),
              workDate: (s === 'pm' ? 'S' : '') + dates[d],
              classId: parseInt(classId, 10),
              locationId: parseInt(locationId, 10) || 1,
              deptId: deptId
            });
          }
        }
  ...
  function interceptScheduleAPI() {
    ...
    const blockedPatterns = [
      '/api/biz/attendance/schedule/employee',
    ...
    window.fetch = function (url, options) {
      ...
      if (isBlocked && (options?.method || 'GET').toUpperCase() !== 'GET') {
        ...
        console.log('[排班辅助] � 拦截的请求体:', options?.body);
        // 提示用户
